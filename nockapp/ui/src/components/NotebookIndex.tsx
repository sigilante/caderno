...
export function NotebookIndex({ notebooks, error, onOpen, onDelete }: Props) {
    ...
    >
      <div style={{ padding: '6px 24px 60px' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 14, margin: '4px 0 22px' }}>
          ...
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: '#6b5a3c', letterSpacing: '.18em' }}>STORE · CHECKPOINT</span>
        </div>
        ...
        {sorted.map((nb, idx) => (
          ...
          >
            ...
            <div style={{
              ...
              {/* A snapshot carries cells only for the *active* notebook, so
                  the count is unknown until this one has been opened. */}
              <span style={{ fontFamily: "'Antonio', sans-serif", fontSize: 30, fontWeight: 700, lineHeight: .9 }}>
                {nb.loaded ? nb.cells.length : '··'}
